...
class Dualis:
    ...
    def login(self, username, password):
        ...
        r = self.session.post(
            ...
            data=f"usrname={self.username}&pass={self.password}&APPNAME=CampusNet&PRGNAME=LOGINCHECK&ARGUMENTS=clino%2Cusrname%2Cpass%2Cmenuno%2Cmenu_type%2Cbrowser%2Cplatform&clino=000000000000001&menuno=000324&menu_type=classic&browser=&platform=",
        )
        if (
            r.status_code != 200
            or r.headers["REFRESH"].split("ARGUMENTS=")[1].split(",")[0] == ""
        ):
    ...
    def get_time_table_week(self, date: date) -> list:
        r = self.session.get(
            f"https://dualis.dhbw.de/scripts/mgrqispi.dll?APPNAME=CampusNet&PRGNAME=SCHEDULER&ARGUMENTS={self.arguments},-N000030,-A{date.strftime('%d/%m/%Y')},-A,-N1,-N0,-N0"
        )
        r.encoding = "utf-8"
        ...
        for appointment in appointments:
            ...
            if appointment_date not in WOCHENTAGE:
                continue
            appointment_date = date + timedelta(
                days=WOCHENTAGE.index(appointment_date) - date.weekday()
            )
